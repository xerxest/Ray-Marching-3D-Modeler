...
int cmdMove(CmdContext* /*_context*/, void* /*_userData*/, int _argc, char const* const* _argv)
{
	if (_argc > 1)
	{
		...
		{
			ViewPortCamera::cameraSetKeyState(CAMERA_KEY_FORWARD, true);
			return 0;
		}
		else if (0 == bx::strCmp(_argv[1], "left") )
		{
			ViewPortCamera::cameraSetKeyState(CAMERA_KEY_LEFT, true);
			return 0;
		}
		else if (0 == bx::strCmp(_argv[1], "right") )
		{
			ViewPortCamera::cameraSetKeyState(CAMERA_KEY_RIGHT, true);
			return 0;
		}
		else if (0 == bx::strCmp(_argv[1], "backward") )
		{
			ViewPortCamera::cameraSetKeyState(CAMERA_KEY_BACKWARD, true);
			return 0;
		}
		else if (0 == bx::strCmp(_argv[1], "up") )
		{
			ViewPortCamera::cameraSetKeyState(CAMERA_KEY_UP, true);
			return 0;
		}
		else if (0 == bx::strCmp(_argv[1], "down") )
		{
			ViewPortCamera::cameraSetKeyState(CAMERA_KEY_DOWN, true);
			return 0;
		}
	}
...
struct Camera
{
	...
	void update(float _deltaTime, const entry::MouseState& _mouseState, bool _reset)
	{
		...
		if (inputGetKeyState(entry::Key::KeyJ))
		{
			const float deltaX = _mouseState.m_mx - m_mouseLastPan.m_mx;
			const float deltaY = _mouseState.m_my - m_mouseLastPan.m_my;

			bx::Vec3 pan = bx::Vec3(deltaX, deltaY, 0.0f);
			
			if(std::abs(deltaX) < 3 && std::abs(deltaY) < 3)
			{
				m_eye = bx::mad(pan, _deltaTime  * m_moveSpeed, m_eye);
				std::cout << pan.x  << "," << pan.y << std::endl;
			}


			m_mouseLastPan.m_mx = _mouseState.m_mx;
			m_mouseLastPan.m_my = _mouseState.m_my;

			setKeyState(entry::Key::KeyJ, false);
		}
...
void ViewPortCamera::cameraCreate()
{
	s_camera = BX_NEW(entry::getAllocator(), Camera);
}

void ViewPortCamera::cameraDestroy()
{
	bx::deleteObject(entry::getAllocator(), s_camera);
	s_camera = NULL;
}

void ViewPortCamera::cameraSetPosition(const bx::Vec3& _pos)
{
	s_camera->setPosition(_pos);
}

void ViewPortCamera::cameraSetHorizontalAngle(float _horizontalAngle)
{
	s_camera->setHorizontalAngle(_horizontalAngle);
}

void ViewPortCamera::cameraSetVerticalAngle(float _verticalAngle)
{
	s_camera->setVerticalAngle(_verticalAngle);
}

void ViewPortCamera::cameraSetKeyState(uint8_t _key, bool _down)
{
	s_camera->setKeyState(_key, _down);
}

void ViewPortCamera::cameraGetViewMtx(float* _viewMtx)
{
	s_camera->getViewMtx(_viewMtx);
}

bx::Vec3 ViewPortCamera::cameraGetPosition()
{
	return s_camera->m_eye;
}

bx::Vec3 ViewPortCamera::cameraGetAt()
{
	return s_camera->m_at;
}

void ViewPortCamera::cameraUpdate(float _deltaTime, const entry::MouseState& _mouseState, bool _reset)
{
